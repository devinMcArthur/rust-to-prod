...
impl ResponseError for SubscribeConfirmError {
    fn status_code(&self) -> StatusCode {
        match self {
            SubscribeConfirmError::UnauthorizedError(_) => StatusCode::UNAUTHORIZED,
...
pub async fn confirm(
    ...
    let id = get_subscriber_id_from_token(&pool, &parameters.subscription_token)
        .await
        .context("Unable to fetch subscriber id from your token")?;

    match id {
        // Non-existing token
        None => Err(SubscribeConfirmError::UnauthorizedError(
            "Invalid subscriber id provided".into(),
        )),
        Some(subscriber_id) => {
