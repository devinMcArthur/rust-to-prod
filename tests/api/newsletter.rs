...
async fn newsletters_are_not_delivered_to_unconfirmed_subscribers() {
    ...
    app.post_successful_login().await;
...
#[tokio::test]
async fn should_be_redirected_from_publish_newsletter_page_if_unauthorized() {
    // Arrange
    let app = spawn_app().await;
...
#[tokio::test]
async fn should_access_newsletter_form_when_logged_in() {
    // Arrange
    let app = spawn_app().await;
    app.post_successful_login().await;

    // Act
    let response = app.get_publish_newsletter_html().await;

    // Assert
    assert!(response.contains("Publish Newsletter Issue"));
...
async fn create_unconfirmed_subscriber(app: &TestApp) -> ConfirmationLinks {
    ...
    // With MockGuard, this mock will only last until
    ...
    // We now inspect the requests received by the mock Sendgrid server
